    ...
    if message.content.startswith("!공지"):
        try:
            try:
                ...
                embed3 = discord.Embed(title=f'{name}', description=f'채널 아이디와 전달할 매세지를 보내주세요.', color=0xFF0000)
                await message.channel.send(embed=embed3)                   
            target = discord.utils.get(message.guild.roles, name=f"{role}") 
            if not target in message.author.roles: 
                return
            if message.author.bot:
                ...
                if msg.content == "아니요":
                    ...
                    if msg.content == "네":                          
                        ...
                        embed3 = discord.Embed(title=f'{name} NOTICE', description=f'네/아니요를 보내주세요. 전달이 취소 되었습니다.', color=0xFF0000)
                        return await message.channel.send(embed=embed3)                                            
